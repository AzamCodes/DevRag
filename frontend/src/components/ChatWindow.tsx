...
export default function ChatWindow() {
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      const sources = data.sources?.length
        ? data.sources
        : extractUrls(cleanText);

      setMessages((prev) =>
        prev.map((m) =>
          m.id === thinkingMsg.id
            ? { ...m, text: cleanText, sources }
            : m
        )
      );
    } catch (err: any) {
  ...
  // Reuse extractUrls from MessageBubble to ensure consistency
  const extractUrls = (text: string): { title: string; url: string }[] => {
    if (!text) return [];
    const urlRegex = /\[([^\]]+)\]\((https?:\/\/[^\s)]+?)\)/g;
    ...
    // Fallback: Extract plain URLs if no Markdown links
    if (sources.length === 0) {
